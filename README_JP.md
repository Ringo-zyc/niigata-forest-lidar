...
| 機能 | 説明 | ツール |
|------|------|------|
| **GUIインターフェース** | タブデザイン、検出＋可視化の一体化 | `gui_app.py` |
| **コマンドライン検出** | 画面なしでのバッチ処理 | `detect_cylinders_v2.py` |
| **3D可視化** | Matplotlib静的画像 + Open3D対話型 | `visualize_forest.py` |
| **精度検証** | 層化抽出 + RMSE/MAE計算 | `generate_validation_sample.py` |
| **バイオマス分析** | 直径階分布、炭素蓄積量推定 | `analyze_forest_data.py` |
...
│   ├── San Juan Fault/            # 主要研究エリア
│   └── StREAM Lab/                # 検証用サブエリア
│
├── 02_Screenshots/                 # 学習プロセスのスクリーンショット
│   └── failures/                  # 失敗事例記録（深層学習データエンジニアリング用）
│
├── 03_Scripts/                     # 🐍 コアスクリプト
│   ├── run.sh                     # 統一エントリーポイント
...
│   └── MANUAL.md                  # ツール使用マニュアル
│
├── 04_Results/                     # 出力結果
│   ├── figures/                   # 図表 (.png)
│   ├── tables/                    # データ表 (.csv)
│   └── reports/                   # 📅 計画・学習ドキュメント
│       ├── path_ab_learning_plan.html      # 深層学習ロードマップ
│       ├── iti_learning_summary.md         # 短期学習サマリー
│       └── graduate_career_roadmap.html    # 2カ年キャリア計画
│
├── docs/                           # 📚 プロジェクト核心ドキュメント
